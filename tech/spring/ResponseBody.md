...
`@ResponseBody`란 컨트롤러 메서드의 리턴값을 그대로 HTTP 응답 body에 담아 보내도록 하는 어노테이션입니다. 그리고 `ResponseEntity<T>`는 HTTP 상태코드, 헤더, body(T) 를 개발자가 직접 설정해서 반환할 수 있는 응답 객체입니다.

## 🤷🏻‍♂️ `@ResponseBody`(or `ResponseEntity<T>)`가 있을 때와 없을 때의 동작 방식의 차이점을 말해주세요.
...
## 🤷🏻‍♂️ `@ResponseBody`와 `ResponseEntity<T>` 반환 중 어떤 방식이 더욱 좋나요?
